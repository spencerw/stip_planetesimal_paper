...
develop a bimodal population dynamically hot planetesimals and
...
investigate that our simplified collision model has on the emergence
...
that the asteroid belt, Kuiper belt and the Oort cloud are largely composed of the leftovers of this stage of planet formation.
...
the gravitational influence of the few massive oligarchs that
form make statistical methods inappropriate. Due to computational expense, N-body simulations of
...
bodies develop, they become increasingly effective at heating the
...
\begin{center}
    ...
    \ref{sec:narrow}. The runaway growth phase can be identified by a positive trend in this ratio. For all values of $\alpha$, an 
    increase in $\beta$ has the effect of delaying runaway growth.\label{fig:alpha_beta_evo}}
\end{center}
\end{figure}
...
rather than scattering. In the case of large $\beta$, the protoplanets
...
embryos \citep{weidenschilling89, ida90}. When $\alpha$ is non-negligible, the Hill sphere of a body is no longer mostly 
empty space, and close encounters that would otherwise result in viscous stirring instead result in accretion. In this regime, we 
...
In all cases, the eccentricities and inclinations of the bodies are randomly drawn from a Rayleigh distribution, with $\left< e^{2} \right>^{1/2} = 2\left<i^{2} \right>^{1/2} = e_{eq}$ \citep{ida93}. The value of $e_{eq}$ is chosen such that the timescales for viscous stirring and aerodynamic gas drag on the planetesimals are in equilibrium. The viscous stirring timescale is given by \citet{ida93} as
